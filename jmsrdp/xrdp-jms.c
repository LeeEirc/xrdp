...
set_keyboard_overrides(struct mod *mod)
{
    ...
    {
        settings->KeyboardType = mod->client_info.keyboard_type;
        settings->KeyboardSubType = mod->client_info.keyboard_subtype;
        /* Define the most common number of function keys, 12.
           because we can't get it from client. */
        settings->KeyboardFunctionKey = 12;
        settings->KeyboardLayout = mod->client_info.keylayout;
    ...
    if (ko->subtype != 0)
    {
        LOG(LOG_LEVEL_INFO, "overrode kbd_subtype 0x%02X with 0x%02X",
            settings->KeyboardSubType, ko->subtype);
        settings->KeyboardSubType = ko->subtype;
    }
    ...
    if (ko->fn_keys != 0)
    {
        LOG(LOG_LEVEL_INFO, "overrode kbd_fn_keys %d with %d",
            settings->KeyboardFunctionKey, ko->fn_keys);
        settings->KeyboardFunctionKey = ko->fn_keys;
    }
    ...
    if (ko->layout != 0)
    {
        LOG(LOG_LEVEL_INFO, "overrode kbd_layout 0x%08X with 0x%08X",
            settings->KeyboardLayout, ko->layout);
        settings->KeyboardLayout = ko->layout;
    }
    ...
    if (ko->layout_mask != 0)
    {
        LOG(LOG_LEVEL_INFO, "Masked kbd_layout 0x%08X to 0x%08X",
            settings->KeyboardLayout, settings->KeyboardLayout & ko->layout_mask);
        settings->KeyboardLayout &= ko->layout_mask;
    }
...
lfreerdp_pre_connect(freerdp *instance)
{
    ...
    instance->settings->OffscreenCacheSize = 0;
    instance->settings->DrawNineGridEnabled = 0;

    instance->settings->GlyphCache = true;
    ...
    instance->settings->OrderSupport[NEG_DSTBLT_INDEX] = 1; /* 0x00 */
    instance->settings->OrderSupport[NEG_PATBLT_INDEX] = 1;
    instance->settings->OrderSupport[NEG_SCRBLT_INDEX] = 1;
    instance->settings->OrderSupport[NEG_MEMBLT_INDEX] = 1;
    instance->settings->OrderSupport[NEG_MEM3BLT_INDEX] = 0;
    instance->settings->OrderSupport[NEG_ATEXTOUT_INDEX] = 0;
    instance->settings->OrderSupport[NEG_AEXTTEXTOUT_INDEX] = 0;
    instance->settings->OrderSupport[NEG_DRAWNINEGRID_INDEX] = 0;
    instance->settings->OrderSupport[NEG_LINETO_INDEX] = 1; /* 0x08 */
    instance->settings->OrderSupport[NEG_MULTI_DRAWNINEGRID_INDEX] = 0;
    instance->settings->OrderSupport[NEG_OPAQUE_RECT_INDEX] = 1;
    instance->settings->OrderSupport[NEG_SAVEBITMAP_INDEX] = 0;
    instance->settings->OrderSupport[NEG_WTEXTOUT_INDEX] = 0;
    instance->settings->OrderSupport[NEG_MEMBLT_V2_INDEX] = 1;
    instance->settings->OrderSupport[NEG_MEM3BLT_V2_INDEX] = 0;
    instance->settings->OrderSupport[NEG_MULTIDSTBLT_INDEX] = 0;
    instance->settings->OrderSupport[NEG_MULTIPATBLT_INDEX] = 0; /* 0x10 */
    instance->settings->OrderSupport[NEG_MULTISCRBLT_INDEX] = 0;
    instance->settings->OrderSupport[NEG_MULTIOPAQUERECT_INDEX] = 0;
    instance->settings->OrderSupport[NEG_FAST_INDEX_INDEX] = 0;
    instance->settings->OrderSupport[NEG_POLYGON_SC_INDEX] = 0;
    instance->settings->OrderSupport[NEG_POLYGON_CB_INDEX] = 0;
    instance->settings->OrderSupport[NEG_POLYLINE_INDEX] = 0;
    /* 0x17 missing */
    instance->settings->OrderSupport[NEG_FAST_GLYPH_INDEX] = 0; /* 0x18 */
    instance->settings->OrderSupport[NEG_ELLIPSE_SC_INDEX] = 0;
    instance->settings->OrderSupport[NEG_ELLIPSE_CB_INDEX] = 0;
    /* disabled as workaround for corrupted display like black bars left of cmd with W2K8 */
    /* instance->settings->OrderSupport[NEG_GLYPH_INDEX_INDEX] = 1; */
    instance->settings->OrderSupport[NEG_GLYPH_INDEX_INDEX] = 0;

    instance->settings->OrderSupport[NEG_GLYPH_WEXTTEXTOUT_INDEX] = 0;
    instance->settings->OrderSupport[NEG_GLYPH_WLONGTEXTOUT_INDEX] = 0;
    instance->settings->OrderSupport[NEG_GLYPH_WLONGEXTTEXTOUT_INDEX] = 0;
    /* 0x1F missing*/
    ...
    instance->settings->BitmapCacheV3Enabled = 1;
    ...
    instance->settings->PerformanceFlags &= ~mod->perf_settings_override_mask;

    /* Add in overridden performance settings */
    instance->settings->PerformanceFlags |= mod->perf_settings_values_mask;

    LOG(LOG_LEVEL_DEBUG, "final performance_flags:[0x%08x]",
        instance->settings->PerformanceFlags);

    instance->settings->CompressionEnabled = 0;
    instance->settings->IgnoreCertificate = 1;

    // Multi Monitor Settings
    instance->settings->MonitorCount = mod->client_info.monitorCount;

    for (index = 0; index < mod->client_info.monitorCount; index++)
    {
        instance->settings->MonitorDefArray[index].x = mod->client_info.minfo[index].left;
        instance->settings->MonitorDefArray[index].y = mod->client_info.minfo[index].top;
        instance->settings->MonitorDefArray[index].width = mod->client_info.minfo[index].right;
        instance->settings->MonitorDefArray[index].height = mod->client_info.minfo[index].bottom;
        instance->settings->MonitorDefArray[index].is_primary = mod->client_info.minfo[index].is_primary;
    }
...
mod_init(void)
{
    ...
    mod->inst->VerifyCertificateEx = lfreerdp_verify_certificate;
#if defined(VERSION_STRUCT_RDP_FREERDP)
